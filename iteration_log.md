...
| 2026-06-28 | Generator | Added resume generator (per-job, controlled fit levels Excellent-Mismatch, rotating 5 styles; parallel + correction loop), companion pairs_*.jsonl linking resumes to jobs, and show_resumes viewer | No resume generation (0) | Resumes + linked pairs (5/5 valid on 1-job test) | resume stage added | Keep |
| 2026-06-29 | Labeler | Added rule-based resume-job evaluator computing the 6 failure metrics per pair (token-set Jaccard skills overlap + experience/seniority mismatch, missing core skills, hallucinated skills, awkward language) writing failure_labels_*.jsonl | initial setup | initial setup | labeler stage added | Keep |
